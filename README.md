...
PostgreSQL은 **도커가 아니라 호스트에 네이티브로** 설치돼 있고, **앱별로 클러스터(인스턴스)를 분리**해서 운영합니다. 한 클러스터에 DB를 여러 개 두면 서로 이름이 보여 격리가 약하기 때문에, 앱마다 별도 포트의 클러스터를 씁니다.

| 클러스터 | 포트 | DB / 유저 | 용도 |
|----------|------|-----------|------|
| `14/main`     | 5432 | comfit   | 다른 앱(comfit) |
| `14/hansolax` | 5433 | hansolax | 이 앱 |

- 두 클러스터는 **프로세스·카탈로그가 완전히 분리**됩니다. comfit(5432) 접속자는 hansolax(5433)의 존재 자체를 볼 수 없고, 반대도 마찬가지.
- 모든 클러스터는 **`127.0.0.1`에만 바인딩**(외부 미노출). 외부에서 붙으려면 **SSH 터널**을 통해야 합니다.
- 클러스터 목록 확인: `pg_lsclusters`

### users 테이블 (hansolax DB)

| 컬럼 | 타입 | 제약 | 설명 |
|------|------|------|------|
| id | SERIAL | PK | 고유 ID(자동 증가) |
| email | VARCHAR(255) | UNIQUE NOT NULL | 이메일(회사 내부 이메일만 허용) |
| password | VARCHAR(255) | NOT NULL | 비밀번호(bcrypt 해싱, `pgcrypto`) |
...
| `server/app/db.py` | asyncpg 풀 생성/해제. **연결 실패해도 앱은 죽지 않음**(경고만, 헬스체크로 확인) |
...
**가장 흔한 함정 — 비번 불일치**: `server/.env`의 비번과 실제 DB 유저 비번이 다르면 `503 DB 미연결` + 컨테이너 로그에 `password authentication failed`가 뜹니다. 둘을 일치시키세요:

```bash
# server/.env 의 값으로 DB 비번을 맞춤
sudo -u postgres psql -p 5433 -c "ALTER USER hansolax WITH PASSWORD '<server/.env와 동일한 값>';"
sudo docker compose -f docker-compose.prod.yml restart api   # 재연결
```
