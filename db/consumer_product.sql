...
CREATE TABLE IF NOT EXISTS consumer_product (
  ...
  resolve_status text NOT NULL DEFAULT 'resolved'
    CHECK (resolve_status IN ('resolved', 'unresolved')),
  context text,
  ...
  CONSTRAINT chk_consumer_product_resolve_fields CHECK (
    (resolve_status = 'resolved' AND product_id IS NOT NULL)
    OR
    (resolve_status = 'unresolved' AND product_id IS NULL AND mentioned_name IS NOT NULL)
  )
);
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_consumer_product_unresolved
  ON consumer_product(content_id, mentioned_name)
