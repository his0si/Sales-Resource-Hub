...
## 영업일지(Sales Memo) 자동 적재

`ai.hansolhomedeco@gmail.com` 메일함으로 오는 **[HSP SalesMemo]** 메일(거래선 방문 영업일지)을 백엔드가 주기적으로 읽어 파싱한 뒤 `sales_memo` 테이블에 자동 적재(upsert)하고, 화면은 Gmail이 아니라 **DB만** 최신순으로 보여줍니다.

**흐름**

1. **메일 읽기** — `server/app/gmail_client.py`가 OAuth2 refresh token으로 Gmail API(`users.messages`)를 호출. (최초 1회 `server/scripts/gmail_authorize.py`로 refresh token 발급 → `.env`)
2. **폴링 적재** — `server/app/sales_memo_sync.py`가 `SALES_MEMO_POLL_SECONDS`(기본 300초)마다 `subject:"HSP SalesMemo"` 메일을 확인. 새 메일만 `sales_memo_parser.py`로 파싱해 upsert. `main.py` lifespan에서 백그라운드 태스크로 기동.
3. **조회** — `GET /api/sales-memo`(로그인 필요)가 `sales_memo` 전체를 **작성일 최신순**으로 반환. 프론트(`Home.tsx`)가 컬럼으로 원본 HSP Sales Memo 표를 재구성해 표시.

**메일 표 라벨 → `sales_memo` 컬럼 매핑**
...
- **중복 방지**: `sales_memo.gmail_id`(메일 메시지 id, 부분 UNIQUE 인덱스). 메일 유입 행의 `visit_no`는 `'GM'+gmail_id`. 기존 xlsx 적재분(378건)은 `gmail_id=NULL`로 그대로 유지 → 한 화면에 함께 최신순 표시.

| 파일 | 역할 |
|------|------|
| `server/app/gmail_client.py` | Gmail API 호출(OAuth2 refresh token) |
| `server/app/routers/gmail.py` | `GET /api/gmail/inbox`·`/messages` (원본 메일 직접 조회용) |
| `server/app/sales_memo_parser.py` | 메일 HTML 표 → 컬럼 dict 파싱(표준 라이브러리만) |
| `server/app/sales_memo_sync.py` | 주기 폴링 + upsert(+메타데이터 보강) |
| `server/app/routers/sales_memo.py` | `GET /api/sales-memo` 최신순 조회 |
| `db/sales_memo.sql` | `sales_memo` 테이블(+`gmail_id` 부분 유니크 인덱스) |
| `db/gmail_messages.sql` | 받은 메일 원문 캐시 테이블(선택) |

> 폴링 동작은 `.env`의 `SALES_MEMO_SYNC_ENABLED`/`SALES_MEMO_POLL_SECONDS`/`SALES_MEMO_QUERY`/`SALES_MEMO_MAX_SCAN`로 조정합니다(기본값으로 동작, 보통 손댈 필요 없음).
...
│   ├── sales_memo.sql          # 영업일지 테이블 ([HSP SalesMemo] 메일 자동 적재 대상)
│   └── gmail_messages.sql      # 받은 메일 원문 캐시 테이블
